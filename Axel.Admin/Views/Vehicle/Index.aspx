...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div id="Div1" class="typography fivecolumn">
        ...
        <a href="<%= Url.Action("WebPage", "Vehicle")%>" class="tab_link">
            <img src="../../Images/NoteBook.png" style="height: 20px; margin: -6px 5px;" />Add a Vehicle</a>
        <div class="table">
            <div class="row clr">
            ...
            <div class="row clr">
                <div class="column">
                    <a href="<%= Url.Action("WebPage", "Vehicle", new { ID = m.SEQ_ID })%>">
                        <img alt="Edit" style="margin: 0px 10px -4px 10px;" src="../../Images/edit_inline.gif" /></a>
                    | <a href="<%= Url.Action("Delete", "Vehicle", new { ID = m.SEQ_ID })%>">
                        <img alt="Delete" style="margin: 0px 10px -4px 10px;" src="../../Images/delete_inline.gif" /></a>
                </div>
                <div class="column">
                    <%= Html.Encode(string.IsNullOrEmpty(m.DESCRIPTION)?"-":m.DESCRIPTION)%>
                </div>
                <div class="column">
                    <%= Html.Encode(((SelectList)ViewData["ManufacturerList"]).Where(i => i.Value == m.MANUFACTURER_SEQ_ID).SingleOrDefault().Text) %>
                </div>
                <div class="column">
                    <%= Html.Encode(((SelectList)ViewData["ClassList"]).Where(i => i.Value == Convert.ToString(m.CLASS_TYPE_SEQ_ID)).SingleOrDefault().Text) %>
                </div>
                <div class="column">
                    <%= Html.Encode(m.ACTIVE)%>
                </div>
            </div>
            <% } %>
        </div>
