        ...
        checkValues: function (val, valref, customobject, nam) {
            ...
            if (edtrul) {
                ...
                if (edtrul.date === true) {
                    ...
                    if (edtrul.Len !== "") {
                        var givenLength = val.toString().length;
                        var requiredLength = edtrul.Len;
                        if (givenLength < requiredLength || val.toString().substring(5, 3) > 12) {
                            return [false, nm + ": " + $.jgrid.edit.msg.Len, ""];
                        }
                    }

                    if (edtrul.minDate !== "") {
                        var startDate = new Date(val.toString().substring(10, 6), val.toString().substring(5, 3) - 1, val.toString().substring(2, 0));
                        var endDate = new Date(edtrul.minDate.toString().substring(10, 6), edtrul.minDate.toString().substring(5, 3) - 1, edtrul.minDate.toString().substring(2, 0));

                        if (startDate < endDate) {
                            return [false, nm + ": " + $.jgrid.edit.msg.minDate + " Date Joined Vessel: " + edtrul.minDate + " and Date Left Vessel: " + edtrul.maxDate, ""];
                        }

                    }
                    if (edtrul.maxDate !== "") {
                        var startDate = new Date(val.toString().substring(10, 6), val.toString().substring(5, 3) - 1, val.toString().substring(2, 0));
                        var endDate = new Date(edtrul.maxDate.toString().substring(10, 6), edtrul.maxDate.toString().substring(5, 3) - 1, edtrul.maxDate.toString().substring(2, 0));

                        if (startDate > endDate) {
                            return [false, nm + ": " + $.jgrid.edit.msg.minDate + " " + edtrul.minDate + " and " + edtrul.maxDate, ""];
                        }
                    }
...
(function ($) {
    ...
    **/
    ...
    $.jgrid.extend({
        ...
        delGridRow: function (rowids, p, RecordId) {
            p = $.extend(true, {
                top: 0,
                left: 0,
                width: 240,
                ...
                if ($("#" + $.jgrid.jqID(IDs.themodal))[0] !== undefined) {
                    ...
                    $("#dData", "#" + dtbl + "_2")
                        ...
                        if (ret[0] && !rp_ge[$t.p.id].processing) {
                            ...
                            var ajaxOptions = $.extend({
                                url: $($t).jqGrid('getGridParam', 'delurl'), //rp_ge[$t.p.id].url || $($t).jqGrid('getGridParam', 'delurl'),                                
                                type: rp_ge[$t.p.id].mtype,
                                data: { Id: RecordId }, // $.isFunction(rp_ge[$t.p].serializeDelData) ? rp_ge[$t.p.id].serializeDelData.call($t, postd) : postd,
                                complete: function (data, status) {
                                    var i;
                                    if (data.status >= 300 && data.status !== 304) {
                                        ret[0] = false;
    ...
    $.jgrid.extend({
        ...
        inlineNav: function (elem, o) {
            o = $.extend(true, {
                ...
                savetext: "Save",
                savetitle: "Save new row",
                cancel: true,
                cancelicon: "ui-icon-cancel",
                canceltext: "Cancel",
                canceltitle: "Cancel new row",
                alertcap: "Warning",
                ...
                if (o.del) {
                    $($t).jqGrid('navButtonAdd', elem, {
                        ...
                        onClickButton: function () {
                            ...
                            var value = $($t).jqGrid('getCell', sr, 'RecordId');
                            if (sr) {                                
                                var pDel = $t.p;
                                $($t).jqGrid("delGridRow", sr, pDel, value);

                            } else {
                                $.jgrid.viewModal("#alertmod_" + $.jgrid.jqID($t.p.id), { gbox: "#gbox_" + $.jgrid.jqID($t.p.id), jqm: true }); $("#jqg_alrt").focus();
                            }
                        }
                    });
                }
